...
const HomeScreen = ({navigation}) => {
  ...
  const getData = async () => {
    ...
    const response = await fetch(
      `https://newsapi.org/v2/top-headlines?country=us&apiKey=8e2460ba1f4a4081a87346690ceaec6a&category=${Category[Select].category}`,
    );
  ...
  const getData2 = async (category) => {
    setLoading(true)
    const response = await fetch(
      `https://newsapi.org/v2/top-headlines?country=us&apiKey=8e2460ba1f4a4081a87346690ceaec6a&category=${category}`,
    );
    const data = await response.json();
    setData(data.articles);
    setLoading(false)
